...
/**
 * Fetches educational investment information for a hypothetical investment scenario
 * @param params Investment parameters
 * @returns Promise with educational investment content
 */
export async function getinvestmentInsights(params: InvestmentParams): Promise<InvestmentInsights> {
  try {
    const response = await axios.post('/api/insights/sip-insights', params);
    return response.data;
  } catch (error) {
    console.error('Error fetching investment educational content:', error);
    
    // Return default educational content in case of error (with cautious language)
    return {
      personalizedAdvice: [
        `A hypothetical monthly investment of $${params.monthlyinvestmentAmount.toLocaleString('en-IN')} for ${params.investmentPeriod} years could potentially be one disciplined approach to financial planning.`,
        `Some investors might consider gradually adjusting their investment amount over time to potentially account for inflation and possible income changes.`,
