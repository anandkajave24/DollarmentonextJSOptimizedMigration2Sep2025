...
import { homeBuyerStages, HomeBuyerGameStage, HomeBuyerStageOption } from '@/data/homebuyers';
import { Home, CreditCard, TrendingUp, Brain, DollarSign, AlertTriangle } from 'lucide-react';
...
const HomeBuyerFIREGame: React.FC = () => {
  ...
  const currentStageData = homeBuyerStages[gameState.currentStage];
  const isGameComplete = gameState.currentStage >= homeBuyerStages.length;

  const handleOptionSelect = (option: HomeBuyerStageOption) => {
    if (selectedOption === option.id) {
      ...
      setGameState(prev => ({
        ...
        creditScore: Math.max(300, Math.min(850, prev.creditScore + option.impact.creditScore)),
        homeEquity: prev.homeEquity + option.impact.homeEquity,
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <Card className="border-blue-200 bg-blue-50">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl text-blue-800">First-Time Home Buyer Journey Complete! 🏠</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">${gameState.homeEquity.toLocaleString()}</div>
            ...
            <div className="mt-6">
              <h3 className="text-lg font-semibold mb-3">Your Home Buying Journey:</h3>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <CardTitle className="text-xl">First-Time Home Buyer Journey</CardTitle>
                <p className="text-gray-600">Stage {gameState.currentStage + 1} of {homeBuyerStages.length}</p>
              </div>
            </div>
          ...
          <Progress value={((gameState.currentStage) / homeBuyerStages.length) * 100} className="mt-4" />
        </CardHeader>
      </Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-blue-600">${gameState.homeEquity.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="text-right">
                    <div className={getCreditScoreColor(gameState.creditScore)}>
                      {gameState.creditScore}
                    </div>
                    <div className="text-xs text-gray-500">
                      {getCreditScoreLabel(gameState.creditScore)}
                    </div>
                  </div>
                </div>
                <Progress value={((gameState.creditScore - 300) / 550) * 100} className="h-2" />
              </div>
                      ...
                      {selectedOption === option.id && (
                        <div className="mt-4 space-y-3 border-t pt-4">
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <div>Credit Score: <span className={option.impact.creditScore >= 0 ? 'text-green-600' : 'text-red-600'}>
                              {option.impact.creditScore >= 0 ? '+' : ''}{option.impact.creditScore}
                            </span></div>
                            <div>Home Equity: <span className={option.impact.homeEquity >= 0 ? 'text-green-600' : 'text-red-600'}>
                              {option.impact.homeEquity >= 0 ? '+' : ''}${option.impact.homeEquity.toLocaleString()}
                            </span></div>
                          </div>
...
export default HomeBuyerFIREGame;