...
export function BudgetProvider({ children }: { children: ReactNode }) {
  ...
  const [budgetData, setBudgetData] = useState<BudgetData>(() => {
    // Check if this is a new session by looking for sessionStorage data only
    // If no sessionStorage data exists, start fresh with default values
    const sessionData = sessionStorage.getItem('budgetData');
  ...
  const [dailyExpenses, setDailyExpenses] = useState<ExpenseItem[]>(() => {
    ...
    const sessionData = sessionStorage.getItem('dailyExpenses');
