...
import { TrendingUp, BarChart3, Building2, Shield, Plus } from "lucide-react";
...
interface LinkedInvestment {
  id: string;
  type: "mutual_fund" | "equity" | "bank_account" | "401k";
  name: string;
  currentValue: number;
  identifier: string; // folio number, account number, etc.
  allocatedAmount?: number; // Amount already allocated to goals
}

// Investment allocation tracking interface
...
interface Goal {
  ...
  savingsPlan: "Fixed Monthly Savings" | "Percentage of Income" | "Round-Up Savings" | "Continue Current Investment";
  savingsAmount: number;
  autoSave: boolean;
  linkedInvestments?: LinkedInvestment[];
}

// Calculate monthly savings needed based on goal details including linked investments
const calculateMonthlySavings = (targetAmount: number, currentSavings: number, months: number, linkedInvestments?: LinkedInvestment[]): number => {
  const linkedInvestmentValue = linkedInvestments ? 
    linkedInvestments.reduce((sum, investment) => sum + (investment.allocatedAmount || investment.currentValue), 0) : 0;
  const totalCurrentAmount = currentSavings + linkedInvestmentValue;
  const remaining = targetAmount - totalCurrentAmount;
  return months > 0 && remaining > 0 ? Math.round(remaining / months) : 0;
};

// Helper function to calculate total allocated amount for an investment across all goals
const calculateTotalAllocatedAmount = (investmentId: string, goals: Goal[]): number => {
  let totalAllocated = 0;
  goals.forEach(goal => {
    if (goal.linkedInvestments) {
      const linkedInvestment = goal.linkedInvestments.find(inv => inv.id === investmentId);
      if (linkedInvestment) {
        totalAllocated += linkedInvestment.allocatedAmount || linkedInvestment.currentValue;
      }
    }
  });
...
// Helper function to get remaining amount for an investment
const getRemainingInvestmentAmount = (investment: LinkedInvestment, goals: Goal[]): number => {
  const totalAllocated = calculateTotalAllocatedAmount(investment.id, goals);
  return Math.max(0, investment.currentValue - totalAllocated);
};

// Helper function to get allocation percentage
const getAllocationPercentage = (allocatedAmount: number, totalValue: number): number => {
  return totalValue > 0 ? Math.round((allocatedAmount / totalValue) * 100) : 0;
};
...
export default function GoalSettings() {
  ...
  const [savingsPlan, setSavingsPlan] = useState<"Fixed Monthly Savings" | "Percentage of Income" | "Round-Up Savings" | "Continue Current Investment">("Fixed Monthly Savings");
  ...
  // Investment linking state
  const [linkedInvestments, setLinkedInvestments] = useState<LinkedInvestment[]>([]);
  const [showInvestmentLinking, setShowInvestmentLinking] = useState(false);
  const [availableInvestments, setAvailableInvestments] = useState<LinkedInvestment[]>([]);
  
  // Partial investment allocation state
  const [showPartialAllocationDialog, setShowPartialAllocationDialog] = useState(false);
  const [partialAllocationData, setPartialAllocationData] = useState<{
    investment: LinkedInvestment;
    goalId: string;
    excessAmount: number;
  ...
  const [investmentAllocations, setInvestmentAllocations] = useState<InvestmentAllocation[]>([]);
  const [showPartialAllocationWarning, setShowPartialAllocationWarning] = useState(false);
  ...
  // Fetch investment data from Account Aggregator
  const fetchInvestmentData = async () => {
    try {
      // Mock data based on American investment structure - in real implementation this would be API calls
      const mockInvestments: LinkedInvestment[] = [
        {
          id: "mf1",
          type: "mutual_fund",
      ...
      setAvailableInvestments(mockInvestments);
    } catch (error) {
  ...
  useEffect(() => {
    if (editingGoalId) {
      ...
      if (goalToEdit) {
        ...
        setLinkedInvestments(goalToEdit.linkedInvestments || []);
      }
    }
  }, [editingGoalId, goals]);
  ...
  const handleSaveGoal = () => {
    ...
    // Calculate actual savings amount based on the selected plan and linked investments
    ...
    } else if (savingsPlan === "Continue Current Investment") {
      // For "Continue Current Investment", we don't need additional monthly savings
    ...
    const months = Math.max(1, Math.ceil((goalDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24 * 30)));
    
    // Calculate intelligent monthly savings based on linked investments
    const intelligentSavingsAmount = calculateMonthlySavings(
      targetAmount,
      currentSavings,
      Math.max(1, months),
      linkedInvestments
    ...
    // Use intelligent calculation if no manual amount is set or if user wants automatic calculation
    if (savingsPlan === "Fixed Monthly Savings" && monthlySavingsAmount === 0) {
      finalSavingsAmount = intelligentSavingsAmount;
    }
  ...
  const resetForm = () => {
    ...
    setShowInvestmentLinking(false);
  ...
  const calculateProgress = (goal: Goal) => {
    const linkedInvestmentValue = goal.linkedInvestments ? 
      goal.linkedInvestments.reduce((sum, investment) => sum + (investment.allocatedAmount || investment.currentValue), 0) : 0;
    const totalCurrentSavings = goal.currentSavings + linkedInvestmentValue;
    return (totalCurrentSavings / goal.targetAmount) * 100;
  };
  ...
  const fetchInvestmentSuggestions = async (goal: Goal) => {
    ...
    try {
      ...
      const defaultSuggestions = [
        ...
        `Option: Liquid index funds - Better returns than savings accounts with high liquidity for ${goal.name}.`,
  ...
  return (
    <>
      <SEO 
        title="Goal Settings & Financial Planning - Set & Track Your Financial Goals"
        description="Set, track, and achieve your financial goals with our comprehensive goal planning tool. Create savings goals, investment targets, and monitor progress with detailed analytics and AI-powered recommendations."
        keywords="financial goals, goal setting, financial planning, savings goals, investment goals, goal tracking, financial goal calculator, personal finance goals, goal achievement tool"
        canonical="https://dollarmento.com/goal-settings"
      />
      ...
      {/* Partial Investment Allocation Dialog */}
      <Dialog open={showPartialAllocationDialog} onOpenChange={setShowPartialAllocationDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Partial Investment Allocation</DialogTitle>
            <DialogDescription>
              You have excess investment amount that can be allocated to other goals.
            </DialogDescription>
          </DialogHeader>
          {partialAllocationData && (
            <div className="space-y-4">
              <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
                <h4 className="font-medium text-sm text-blue-800 dark:text-blue-200 mb-2">Investment Details</h4>
                <div className="text-sm space-y-1">
                  <div>Investment: {partialAllocationData.investment.name}</div>
                  <div>Current Goal: {partialAllocationData.currentGoalName}</div>
                  <div className="font-medium text-green-600 dark:text-green-400">
                    Excess Amount: ${partialAllocationData.excessAmount.toLocaleString()}
                  </div>
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  This excess amount can be used to create a new goal or allocated to existing goals.
                </p>
              </div>
            </div>
          )}
          <DialogFooter className="flex space-x-2">
            <Button
              variant="outline"
              onClick={() => setShowPartialAllocationDialog(false)}
            >
              Cancel
            </Button>
            <Button
              onClick={() => {
                ...
                setActiveTab("add");
                if (partialAllocationData) {
                  setGoalType("Custom Goal");
                  setTargetAmount(partialAllocationData.excessAmount);
                  setCustomGoalName("Additional Goal");
                }
              }}
      ...
      {/* Partial Allocation Warning Dialog */}
      <Dialog open={showPartialAllocationWarning} onOpenChange={setShowPartialAllocationWarning}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Partial Investment Allocation</DialogTitle>
            <DialogDescription>
              Your target amount is less than the available investment value.
            </DialogDescription>
          </DialogHeader>
          {allocationWarningData && (
            <div className="space-y-4">
              <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg">
                <h4 className="font-medium text-sm text-orange-800 dark:text-orange-200 mb-2">Investment Details</h4>
                <div className="text-sm space-y-1">
                  <div>Investment: {allocationWarningData.investment.name}</div>
                  <div>Available Amount: ${getRemainingInvestmentAmount(allocationWarningData.investment, goals).toLocaleString()}</div>
                  <div>Target Goal Amount: ${allocationWarningData.targetAmount.toLocaleString()}</div>
                  <div className="font-medium text-orange-600 dark:text-orange-400">
                    Will use {allocationWarningData.allocationPercentage}% of investment
                  </div>
                  <div className="font-medium text-blue-600 dark:text-blue-400">
                    Remaining: ${allocationWarningData.remainingAmount.toLocaleString()} available for other goals
                  </div>
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Consider partial assignment and allocate remaining ${allocationWarningData.remainingAmount.toLocaleString()} to other goals.
                </p>
              </div>
            </div>
          )}
          <DialogFooter className="flex space-x-2">
            <Button
              variant="outline"
              onClick={() => setShowPartialAllocationWarning(false)}
            >
              Cancel
            </Button>
            <Button
              onClick={() => {
                if (allocationWarningData) {
                  // Create investment with partial allocation
                  const investmentWithPartialAllocation = {
                    ...allocationWarningData.investment,
                    allocatedAmount: allocationWarningData.targetAmount
                  };
                  setLinkedInvestments(prev => [...prev, investmentWithPartialAllocation]);
                  setShowPartialAllocationWarning(false);
                }
              }}
              className="bg-orange-600 hover:bg-orange-700"
            >
              Proceed with Partial Allocation
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
                      ...
                      <div className="space-y-4">
                        {categoryGoals.map((goal, index) => {
                          ...
                          const linkedInvestmentValue = goal.linkedInvestments ? 
                            goal.linkedInvestments.reduce((sum, investment) => sum + (investment.allocatedAmount || investment.currentValue), 0) : 0;
                          const totalCurrentSavings = goal.currentSavings + linkedInvestmentValue;
                          const isOverAchieved = totalCurrentSavings >= goal.targetAmount;
                            ...
                            <div key={goal.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg">
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <div className={`text-sm font-medium px-2 py-0.5 rounded-full ${
                                  ...
                                  {isOverAchieved ? "100%" : `${progress.toFixed(1)}%`}
                                </div>
                              </div>
                              <Progress 
                                value={Math.min(progress, 100)} 
                                className="h-3 mb-2 bg-green-100 dark:bg-green-900 progress-green"
                              />
                              ...
                              <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                                <div>
                                  Progress: ${totalCurrentSavings.toLocaleString()} of ${goal.targetAmount.toLocaleString()}
                                </div>
                                {linkedInvestmentValue > 0 && (
                                  <div className="text-xs">
                                    <span className="text-gray-500">Manual: ${goal.currentSavings.toLocaleString()}</span>
                                    <span className="text-blue-600 dark:text-blue-400 ml-3">Investments: ${linkedInvestmentValue.toLocaleString()}</span>
                                  </div>
                                )}
                              </div>
                            </div>
          ...
          <TabsContent value="goals" className="space-y-6">
            {goals.length === 0 ? (
              ...
              goals.map((goal, index) => {
                ...
                // For pie chart: if over-achieved, show 100% saved and 0% remaining
                const pieData = isOverAchieved ? [
                  { name: 'Saved', value: goal.targetAmount },
                  { name: 'Remaining', value: 0 },
                ] : [
                  { name: 'Saved', value: totalCurrentSavings },
                ...
                return (
                  ...
                  >
                    ...
                    <CardContent className="p-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                          <div>
                            <Progress 
                              value={Math.min(progress, 100)} 
                              className="h-4 mb-2 bg-green-100 dark:bg-green-900 progress-green"
                            />
                            <div className="flex justify-between text-sm">
                              <div className="text-gray-600 dark:text-gray-400">$0</div>
                              <div className={`font-medium ${
                                isOverAchieved 
                                  ? 'text-green-600 dark:text-green-400' 
                                  : progress >= 50 
                              ...
                              <div className="text-gray-600 dark:text-gray-400">${goal.targetAmount.toLocaleString()}</div>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg">
                              <div className="text-xs text-gray-500 mb-1">
                                {goal.linkedInvestments && goal.linkedInvestments.length > 0 ? "Monthly Investment" : "Monthly Savings"}
                              </div>
                              <div className="font-medium">
                                {(() => {
                                  if (isOverAchieved) return "$0";
                                  if (goal.linkedInvestments && goal.linkedInvestments.length > 0) {
                                    // For linked investments, show a reasonable monthly investment amount
                                    const linkedValue = goal.linkedInvestments.reduce((sum, inv) => sum + (inv.allocatedAmount || inv.currentValue), 0);
                                    const remainingToGoal = goal.targetAmount - goal.currentSavings - linkedValue;
                                    if (remainingToGoal <= 0) {
                                      return "Continue Current";
                                    }
                                    // Calculate monthly investment needed (assuming 12% annual return)
                                    const currentDate = new Date();
                                    const goalDate = new Date(goal.targetDate);
                                    const months = Math.max(1, Math.ceil((goalDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24 * 30)));
                                    const monthlyInvestment = Math.round(remainingToGoal / months);
                                    return `$${monthlyInvestment.toLocaleString()}`;
                                  }
                                  return `$${goal.savingsAmount.toLocaleString()}`;
                                })()}
                              </div>
                            </div>
                            <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg">
                          ...
                          {/* Linked Investments Section - Compact */}
                          {goal.linkedInvestments && goal.linkedInvestments.length > 0 && (
                            <div className="bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg">
                              <h4 className="text-xs font-medium text-blue-800 dark:text-blue-200 mb-2">
                                Linked Investments ({goal.linkedInvestments.length})
                              </h4>
                              <div className="space-y-1">
                                {goal.linkedInvestments.map((investment) => (
                                  <div key={investment.id} className="flex items-center justify-between py-1 px-2 bg-white dark:bg-gray-800 rounded text-xs">
                                    <div className="flex items-center space-x-2">
                                      {investment.type === "mutual_fund" && <TrendingUp className="w-3 h-3 text-green-500" />}
                                      {investment.type === "equity" && <BarChart3 className="w-3 h-3 text-blue-500" />}
                                      {investment.type === "bank_account" && <Building2 className="w-3 h-3 text-gray-500" />}
                                      {investment.type.toString() === "nps" && <Shield className="w-3 h-3 text-purple-500" />}
                                      <div>
                                        <div className="font-medium">{investment.name}</div>
                                        <div className="text-gray-500 capitalize text-xs">{investment.type.replace('_', ' ')}</div>
                                      </div>
                                    </div>
                                    <div className="text-right">
                                      <div className="font-medium">${(investment.allocatedAmount || investment.currentValue).toLocaleString()}</div>
                                      {investment.allocatedAmount && investment.allocatedAmount < investment.currentValue && (
                                        <div className="text-orange-600 dark:text-orange-400 text-xs">
                                          Partial ({getAllocationPercentage(investment.allocatedAmount, investment.currentValue)}%)
                                        </div>
                                      )}
                                    </div>
                                  </div>
                              ...
                              <div className="mt-2 pt-2 border-t border-blue-200 dark:border-blue-700">
                                <div className="flex justify-between text-xs">
                                  ...
                                  <span className="font-medium text-blue-800 dark:text-blue-200">
                                    ${goal.linkedInvestments.reduce((sum, inv) => sum + (inv.allocatedAmount || inv.currentValue), 0).toLocaleString()}
                                  </span>
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="bg-white dark:bg-gray-900 p-4 rounded-lg flex flex-col h-full">
                          <div className="flex flex-col">
                            ...
                            <div className="flex justify-between text-sm mb-2 px-4">
                              <div className="font-medium flex items-center">
                                ...
                                {isOverAchieved ? "100%" : `${progress.toFixed(0)}%`}
                              </div>
                              <div className="font-medium flex items-center">
                                ...
                                {isOverAchieved ? "0%" : `${(100 - progress).toFixed(0)}%`}
                              </div>
                            </div>
                                ...
                                <div className="bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 rounded-lg p-2">
                                  <div className="text-sm space-y-2">
                                    <span className="text-green-700 dark:text-green-300 font-medium">
                                      Over-allocated by ${(totalCurrentSavings - goal.targetAmount).toLocaleString()} ({(progress - 100).toFixed(0)}% above target)
                                    </span>
                                  </div>
                                </div>
                              </div>
                            ...
                            {/* Compact Savings Breakdown - Horizontal */}
                            {linkedInvestmentValue > 0 && (
                              <div className="mt-2 px-4">
                                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 border">
                                  <div className="flex justify-between items-center text-xs">
                                    <div className="flex items-center">
                                      <span className="text-gray-600 dark:text-gray-400">Manual:</span>
                                      <span className="font-medium ml-1">${goal.currentSavings.toLocaleString()}</span>
                                    </div>
                                    <div className="flex items-center">
                                      <span className="text-blue-600 dark:text-blue-400">Investments:</span>
                                      <span className="font-medium text-blue-600 dark:text-blue-400 ml-1">${linkedInvestmentValue.toLocaleString()}</span>
                                    </div>
                                    <div className="flex items-center">
                                      <span className="text-gray-800 dark:text-gray-200">Total:</span>
                                      <span className="font-bold ml-1">${totalCurrentSavings.toLocaleString()}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
                          ...
                          <div className={`mt-auto p-3 rounded-lg text-sm ${
                            ...
                            <div>
                              {(() => {
                                const hasLinkedInvestments = goal.linkedInvestments && goal.linkedInvestments.length > 0;
                                const investmentValue = hasLinkedInvestments ? 
                                  goal.linkedInvestments!.reduce((sum, inv) => sum + (inv.allocatedAmount || inv.currentValue), 0) : 0;
                                const manualSavings = goal.currentSavings;
                                const totalProgress = progress;
                                
                                if (isOverAchieved) {
                                  if (progress > 100) {
                                    return `Great job! You've exceeded your goal by ${(progress - 100).toFixed(0)}%. Consider setting a higher target or allocating excess funds to other goals.`;
                                  }
                                  return "Congratulations! You've achieved your goal!";
                                }
                                
                                // Dynamic messages based on progress and investment status
                                if (totalProgress === 0) {
                                  if (hasLinkedInvestments) {
                                    return `You've linked $${investmentValue.toLocaleString()} in investments but haven't added manual savings yet. Consider starting monthly contributions to accelerate progress.`;
                                  } else {
                                    return "Just getting started! Consider linking existing investments or increasing your monthly contributions to stay on track.";
                                  }
                                } else if (totalProgress < 25) {
                                  if (hasLinkedInvestments && investmentValue > manualSavings) {
                                    return `Your linked investments ($${investmentValue.toLocaleString()}) are contributing well! Keep up the monthly savings to maintain momentum.`;
                                  } else if (hasLinkedInvestments) {
                                    return `Good start with investments! Consider increasing your monthly contributions or linking more investments to accelerate progress.`;
                                  } else {
                                    return "Just getting started! Consider linking existing investments or increasing your monthly contributions to stay on track.";
                                  }
                                } else if (totalProgress < 50) {
                                  if (hasLinkedInvestments) {
                                    return `Good progress! Your investment strategy is working. Continue with consistent monthly savings to reach your goal faster.`;
                                  } else {
                                    return "Good progress! Consider linking existing investments to boost your goal achievement and reduce monthly savings needed.";
                                  }
                                } else if (totalProgress < 75) {
                                  if (hasLinkedInvestments) {
                                    return `Excellent progress! Your investment approach is working well. You're on track to achieve this goal.`;
                                  } else {
                                    return "Excellent progress! You're more than halfway there. Consider linking investments to reduce remaining monthly burden.";
                                  }
                                } else {
                                  if (hasLinkedInvestments) {
                                    return `Almost there! Your investment strategy has you well-positioned to achieve this goal soon.`;
                                  } else {
                                    return "Almost there! A small boost through linked investments or increased savings will help you reach your goal.";
                                  }
                                }
                              })()}
                            </div>
                      ...
                      {/* Investment Suggestions - Compact Single Line */}
                      <div className="bg-gray-100 dark:bg-gray-800 p-2 mt-4 rounded-none mx-0 mb-0">
                        <div className="text-xs text-gray-600 dark:text-gray-400">
                          <span className="font-medium">Suggestions: </span>
                          {goal.name === "Emergency Fund" ? (
                            "High-yield Savings Account • Liquid Mutual Funds • Short-term FDs"
                          ) : goal.name === "Buying a Gadget" || goal.name === "Vacation & Travel" ? (
                            "Recurring Deposits • Short-term Debt Funds • Automatic Savings Transfers"
                          ) : goal.name === "Buying a Vehicle" ? (
                            "Fixed Deposits • Balanced Mutual Funds • Corporate Bonds"
                          ) : goal.name === "Retirement Fund" || goal.name === "Child's Higher Education" ? (
                            "PPF Account • Equity Mutual Funds • NPS Contributions"
                          ) : (
                            "Balanced Debt-Equity • Index Fund investments • Goal-based Investment Plans"
                          )}
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Choose Goal Type</h4>
                    </div>
                    <RadioGroup 
                      value={goalType} 
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Goal Details</h4>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <Label htmlFor="priority" className="mb-2 block">Goal Priority</Label>
                          <Select value={priority} onValueChange={(value) => setPriority(value as "High" | "Medium" | "Low")}>
                            <SelectTrigger id="priority" className="bg-gray-50 border-2 border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                  ...
                  {/* Step 3: Link Investments (Optional) */}
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3 mb-4">
                      <div className="flex-shrink-0 w-8 h-8 bg-gray-500 text-white rounded-full flex items-center justify-center font-bold text-sm">
                        3
                      ...
                      <div>
                        <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Link Existing Investments</h4>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Optional - Connect your investments to track progress</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <Button
                        type="button"
                        variant="outline"
                        onClick={() => setShowInvestmentLinking(!showInvestmentLinking)}
                        className="w-full"
                      >
                        <TrendingUp className="h-4 w-4 mr-2" />
                        {showInvestmentLinking ? 'Hide' : 'Show'} Available Investments
                      </Button>
                      
                      {showInvestmentLinking && (
                        <div className="border rounded-lg p-4 bg-gray-50 dark:bg-gray-800">
                          <div className="grid grid-cols-1 gap-3 max-h-60 overflow-y-auto">
                            {availableInvestments.length === 0 ? (
                              <div className="text-center text-gray-500 py-4">
                                <p>No investments found. Visit <a href="/my-financial-data" className="text-blue-500 hover:underline">My Financial Data</a> to refresh your portfolio.</p>
                              </div>
                            ) : (
                              availableInvestments.map((investment) => {
                                const isLinked = linkedInvestments.some(linked => linked.id === investment.id);
                                return (
                                  <div key={investment.id} className="flex items-center justify-between p-3 bg-white dark:bg-gray-700 rounded border">
                                    <div className="flex-1">
                                      <div className="flex items-center gap-2">
                                        {investment.type === 'mutual_fund' && <TrendingUp className="h-4 w-4 text-blue-500" />}
                                        {investment.type === 'equity' && <BarChart3 className="h-4 w-4 text-green-500" />}
                                        {investment.type === 'bank_account' && <Building2 className="h-4 w-4 text-purple-500" />}
                                        {investment.type.toString() === 'nps' && <Shield className="h-4 w-4 text-orange-500" />}
                                        <div>
                                          <p className="font-medium text-sm">{investment.name}</p>
                                          {(() => {
                                            const remainingAmount = getRemainingInvestmentAmount(investment, goals);
                                            const totalAllocated = calculateTotalAllocatedAmount(investment.id, goals);
                                            const allocationPercentage = getAllocationPercentage(totalAllocated, investment.currentValue);
                                            
                                            return (
                                              <div className="text-xs space-y-0.5">
                                                <div className="text-gray-500">
                                                  {investment.identifier} • ${investment.currentValue.toLocaleString()}
                                                </div>
                                                {totalAllocated > 0 && (
                                                  <div className="text-orange-600 dark:text-orange-400">
                                                    Available: ${remainingAmount.toLocaleString()} ({(100 - allocationPercentage)}% of total)
                                                  </div>
                                                )}
                                              </div>
                                            );
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        if (isLinked) {
                                          setLinkedInvestments(prev => prev.filter(linked => linked.id !== investment.id));
                                        } else {
                                          // Check if target amount is less than investment value
                                          const linkedValue = linkedInvestments.reduce((sum, inv) => sum + inv.currentValue, 0);
                                          const totalGoalValue = targetAmount;
                                          const remainingInvestmentAmount = getRemainingInvestmentAmount(investment, goals);
                                          
                                          if (totalGoalValue < remainingInvestmentAmount) {
                                            // Calculate allocation percentage and remaining amount
                                            const allocationPercentage = getAllocationPercentage(totalGoalValue, remainingInvestmentAmount);
                                            const excessAmount = remainingInvestmentAmount - totalGoalValue;
                                            
                                            setAllocationWarningData({
                                              investment: investment,
                                              targetAmount: totalGoalValue,
                                              remainingAmount: excessAmount,
                                              allocationPercentage: allocationPercentage
                                            });
                                            setShowPartialAllocationWarning(true);
                                          } else {
                                            // Normal linking - allocate the full remaining amount or target amount, whichever is less
                                            const allocationAmount = Math.min(totalGoalValue - linkedValue, remainingInvestmentAmount);
                                            const investmentWithAllocation = {
                                              ...investment,
                                              allocatedAmount: allocationAmount
                                            };
                                            setLinkedInvestments(prev => [...prev, investmentWithAllocation]);
                                          }
                                        }
                                      }}
                                    >
                          ...
                          {linkedInvestments.length > 0 && (
                            <div className="mt-3 p-3 bg-green-50 dark:bg-green-900 rounded">
                              <p className="text-sm text-green-700 dark:text-green-100 font-medium mb-2">
                                Linked Investments ({linkedInvestments.length}):
                              </p>
                              <div className="space-y-1">
                                {linkedInvestments.map(investment => (
                                  <div key={investment.id} className="text-xs text-green-600 dark:text-green-200">
                                    • {investment.name}: ${(investment.allocatedAmount || investment.currentValue).toLocaleString()}
                                    {investment.allocatedAmount && investment.allocatedAmount < investment.currentValue && (
                              ...
                              <div className="mt-2 pt-2 border-t border-green-200 dark:border-green-700">
                                <p className="text-sm text-green-700 dark:text-green-100 font-medium">
                                  Total Linked Value: ${linkedInvestments.reduce((sum, inv) => sum + (inv.allocatedAmount || inv.currentValue), 0).toLocaleString()}
                                </p>
                              </div>
                            </div>
                          )}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Choose Savings Plan</h4>
                    </div>
                    ...
                    {(() => {
                      ...
                      const goalDate = new Date(targetDate);
                      const months = Math.max(1, Math.ceil((goalDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24 * 30)));
                      const recommendedAmount = calculateMonthlySavings(targetAmount, currentSavings, months, linkedInvestments);
                      const linkedValue = linkedInvestments.reduce((sum, inv) => sum + (inv.allocatedAmount || inv.currentValue), 0);
                        ...
                        return (
                          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-3 mb-4">
                            <div className="flex items-start space-x-2">
                              ...
                              <div className="text-sm">
                                <div className="font-medium text-blue-800 dark:text-blue-200">Smart Recommendation</div>
                                <div className="text-blue-700 dark:text-blue-300 mt-1">
                                  With ${linkedValue.toLocaleString()} from linked investments, you only need {recommendedAmount > 0 ? `$${recommendedAmount.toLocaleString()} monthly` : 'no additional monthly savings'} to reach your goal.
                                </div>
                              </div>
                            </div>
                          </div>
                    ...
                    <RadioGroup 
                      value={savingsPlan} 
                      onValueChange={(value) => setSavingsPlan(value as "Fixed Monthly Savings" | "Percentage of Income" | "Round-Up Savings" | "Continue Current Investment")}
                      className="space-y-3"
                    >
                      {/* Continue Current Investment Option - Show when there are linked investments */}
                      {linkedInvestments.length > 0 && (
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="Continue Current Investment" id="continue" />
                          <Label htmlFor="continue" className="text-green-700 dark:text-green-300 font-medium">
                            Continue Current Investment
                          </Label>
                        </div>
                      )}
                      
                      {savingsPlan === "Continue Current Investment" && (
                        <div className="ml-6 mb-3 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded">
                          <div className="text-sm text-green-700 dark:text-green-300">
                            {(() => {
                              const linkedValue = linkedInvestments.reduce((sum, inv) => sum + (inv.allocatedAmount || inv.currentValue), 0);
                              const remainingAmount = targetAmount - currentSavings - linkedValue;
                              const percentageCovered = ((linkedValue + currentSavings) / targetAmount) * 100;
                              
                              if (remainingAmount <= 0) {
                                return (
                                  <>
                                    <div className="font-medium mb-2">✓ Perfect! Your current investments are sufficient</div>
                                    <div className="text-green-600 dark:text-green-400">
                                      Your linked investments of ${linkedValue.toLocaleString()} already cover your goal amount. 
                                      Just continue with your current investment strategy.
                                    </div>
                                  </>
                                );
                              } else {
                                return (
                                  <>
                                    <div className="font-medium mb-2">✓ Good start with your investment portfolio</div>
                                    <div className="text-green-600 dark:text-green-400">
                                      Your linked investments of ${linkedValue.toLocaleString()} cover {percentageCovered.toFixed(1)}% of your goal. 
                                      Continue investing ${Math.round(remainingAmount / Math.max(1, Math.ceil((new Date(targetDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24 * 30)))).toLocaleString()} monthly to reach your target.
                                    </div>
                                  </>
                                );
                              }
