...
import {
  ...
  BadgePercent, Database, RefreshCw
} from "lucide-react";
...
export default function HomeDownPaymentCalculator() {
  ...
  return (
    <>
      <SEO 
        title="Home Down Payment Calculator - How Much Down Payment for House?"
        description="Calculate required down payment for home purchase. Plan savings goals, compare down payment percentages, and understand PMI requirements for home buying."
        keywords="down payment calculator, home down payment calculator, house down payment calculator, how much down payment for house, home buying down payment, mortgage down payment calculator"
        canonical="https://dollarmento.com/home-down-payment-calculator"
      />
      <HomeDownPaymentCalculatorContent />
    </>
  );
}

function HomeDownPaymentCalculatorContent() {
  const { toast } = useToast();
  const { budgetData, updateExpense } = useBudget();
  const { monthlyIncome, expenses } = budgetData;
  
  // State for property details
  const [propertyValue, setPropertyValue] = useState(5000000); // 50 thousands
  ...
  const [currentSavings, setCurrentSavings] = useState(500000); // 5 thousands
  ...
  const [aaHomeSavingsData, setAAHomeSavingsData] = useState({
    currentSavings: 850000,
    monthlyIncome: 85000,
    monthlySavings: 45000,
    existingInvestments: 1200000,
    targetPropertyValue: 4500000,
  ...
  const loadAAFinancialData = () => {
    ...
    setAAHomeSavingsData(mockAAData);
    setAADataConnected(true);

    // Auto-populate calculator with real data
    setCurrentSavings(mockAAData.currentSavings);
    setMonthlySavings(mockAAData.monthlySavings);
    setPropertyValue(mockAAData.targetPropertyValue);

    toast({
      title: "Real Home Savings Data Loaded",
      description: "Your calculator now shows real-time data from My Financial Data",
    });
  };
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <div className={`mb-3 p-2 rounded ${timeToGoal > 3 && timeToGoal <= 5 ? 'bg-green-50 border border-green-100' : ''}`}>
                                ...
                                <p className="text-sm text-gray-600">Debt index funds, corporate deposits, and balanced funds</p>
                              ...
                              <div className={`p-2 rounded ${timeToGoal > 5 ? 'bg-green-50 border border-green-100' : ''}`}>
                                ...
                                <p className="text-sm text-gray-600">Equity index funds, index funds, and PPF</p>
                                {timeToGoal > 5 && (
                                  <div className="mt-2 flex items-start bg-green-100 p-2 rounded">
                                    <Info className="h-4 w-4 text-green-600 mr-2 mt-0.5 flex-shrink-0" />
                                    <p className="text-xs text-green-700">
                      ...
                      <CardContent>
                        <div className="space-y-4 text-sm">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">Debt index funds, corporate deposits, and balanced funds</p>
                          </div>
                          <div>
                            <h4 className="font-medium mb-1">Long-term goals (5+ years)</h4>
                            <p className="text-gray-600">Equity index funds, index funds, and PPF</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <div className="space-y-3 mt-4">
                      <div className="flex items-start text-sm">
                        ...
                        <div>
                          ...
                          Includes stocks, equity index funds, ETFs.
                        </div>
                      </div>
                      <div className="flex items-start text-sm">
