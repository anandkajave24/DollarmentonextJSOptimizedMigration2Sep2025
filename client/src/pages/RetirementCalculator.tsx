...
import {
  ...
  RefreshCw, Brain, LifeBuoy, Wallet, Award, Zap, Settings, Database
} from "lucide-react";
...
export default function RetirementCalculator() {
  ...
  // AA Financial Data State - Real portfolio data from My Financial Data
  const [aaDataConnected, setAADataConnected] = useState(false);
  const [aaPortfolioData, setAAPortfolioData] = useState({
  ...
  const [existingSavings, setExistingSavings] = useState(1500000); // 15 K existing retirement savings
  ...
  const [monthlyContribution, setMonthlyContribution] = useState(15000); // Realistic investment amount
  ...
  const [requiredMonthlyinvestment, setRequiredMonthlyinvestment] = useState(0);
  ...
  const loadAAFinancialData = () => {
    ...
    // In actual implementation, this would fetch from backend API
    const mockAAData = {
      currentRetirementCorpus: 330000, // NPS (180,000) + Long-term MF (150,000)
      monthlyIncome: 85000, // From banking transaction analysis
      monthlyExpenses: 52000, // From transaction categorization
      existinginvestments: 18500, // Current investment amounts from MF data
      npsBalance: 180000,
      mutualFundsValue: 350000,
      equityValue: 125000,
      fixedDepositsValue: 450000,
      lastUpdated: '23/06/2025 15:30'
    };

    setAAPortfolioData(mockAAData);
    setAADataConnected(true);

    // Auto-populate calculator with real data
    setExistingSavings(mockAAData.currentRetirementCorpus);
    setMonthlyExpenses(mockAAData.monthlyExpenses);
    setMonthlyContribution(mockAAData.existinginvestments);

    toast({
      title: "Real Portfolio Data Loaded",
      description: "Your retirement calculator is now showing real-time data from My Financial Data",
    });
  };
  ...
  const lifestyleTemplates: LifestyleTemplate[] = [
    ...
    },
    {
      ...
      description: "Keep living the way you do now, with some indulgences and trips every now and then.",
      multiplier: 1.0,
    },
    {
      id: "premium",
  ...
  const calculateRetirementRequirements = () => {
    ...
    // Using investment formula: FV = P * ((1 + r)^n - 1) / r * (1 + r)
    ...
    // Calculate future value of monthly investment contributions
    if (annualIncreaseRate === 0) {
      // Without step-up
      ...
      let currentinvestment = monthlyContribution;
      let currentCorpus = futureValueOfExistingSavings;
      
      for (let year = 1; year <= yearsToRetirement; year++) {
        // Add monthly contributions for this year
        for (let month = 1; month <= 12; month++) {
          currentCorpus = currentCorpus * (1 + monthlyPreReturnRate) + currentinvestment;
        }
        
        // Increase investment for next year
        if (year < yearsToRetirement) {
          currentinvestment *= (1 + annualIncreaseRate/100);
        }
      }
    ...
    for (let year = 0; year <= yearsToRetirement; year++) {
      ...
      if (year < yearsToRetirement) {
        ...
        for (let month = 1; month <= 12; month++) {
          yearEndCorpus = yearEndCorpus * (1 + monthlyPreReturnRate) + currentinvestment;
        }
        
        yearlyCorpus = yearEndCorpus;
        
        // Step up investment for next year
        if (annualIncreaseRate > 0) {
          currentinvestment *= (1 + annualIncreaseRate/100);
        }
      }
    ...
    currentinvestment = monthlyContribution;
    ...
    for (let year = 0; year <= yearsToRetirement; year++) {
      ...
      if (year < yearsToRetirement) {
        ...
        yearlyContribution = currentinvestment * 12;
      }
      ...
      if (year < yearsToRetirement) {
        ...
        for (let month = 1; month <= 12; month++) {
          ...
          yearEndCorpus = yearEndCorpus * (1 + monthlyPreReturnRate) + currentinvestment;
          const monthReturn = yearEndCorpus - monthStart - currentinvestment;
    ...
    setRequiredMonthlyinvestment(requiredMonthlyinvestment);
  ...
  // Don't auto-calculate - only calculate when user clicks the button
  ...
  const createRetirementGoal = () => {
    ...
    const retirementGoal = {
      ...
      savingsPlan: annualIncreaseRate > 0 ? "Step-Up investment" : "Fixed Monthly Savings"
    };
  ...
  const getPersonalizedInsights = () => {
    ...
    if (shortfallOrSurplus < 0) {
      ...
      const additionalinvestment = Math.ceil((requiredMonthlyinvestment - monthlyContribution) / 1000) * 1000;
      insights.push(`In similar situations, some retirement planners might consider reviewing their monthly contribution amounts. A difference of around ${formatCurrency(additionalinvestment)} could potentially affect long-term outcomes, though individual circumstances vary greatly.`);
  ...
  return (
    <CalculatorLayout 
      ...
      keywords="retirement planning, retirement calculator, corpus planning, retirement corpus, investment for retirement, pension planning, retirement age, financial independence"
      icon="calculate"
    >
      <div className="mb-6">
        {/* AA Data Integration Status */}
        {aaDataConnected && (
          <Card className="p-6 mb-8 bg-white border-emerald-200">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <Database className="h-5 w-5 text-emerald-600" />
                <div>
                  <h3 className="text-lg font-semibold text-slate-800">Your Live Portfolio Overview</h3>
                  <p className="text-sm text-slate-600">Real-time data from Account Aggregator integration</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100">
                <p className="text-sm text-emerald-700 font-medium mb-1">Retirement Corpus</p>
                <p className="text-2xl font-bold text-slate-900">${(aaPortfolioData.currentRetirementCorpus / 100000).toFixed(1)}L</p>
                <p className="text-xs text-slate-500">NPS + Long-term funds</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                <p className="text-sm text-slate-700 font-medium mb-1">Monthly investments</p>
                <p className="text-2xl font-bold text-slate-900">${(aaPortfolioData.existinginvestments / 1000).toFixed(1)}K</p>
              ...
              <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100">
                ...
                <p className="text-2xl font-bold text-slate-900">${(aaPortfolioData.monthlyIncome / 1000).toFixed(0)}K</p>
                <p className="text-xs text-slate-500">From banking analysis</p>
              </div>
              <div className="bg-red-50 p-4 rounded-lg border border-red-100">
                <p className="text-sm text-red-700 font-medium mb-1">Monthly Expenses</p>
                <p className="text-2xl font-bold text-slate-900">${(aaPortfolioData.monthlyExpenses / 1000).toFixed(0)}K</p>
                <p className="text-xs text-slate-500">Transaction analysis</p>
              </div>
            </div>

            {/* Portfolio Breakdown */}
            <div className="border-t border-gray-200 pt-4">
              <h4 className="text-sm font-medium text-slate-700 mb-3">Asset Allocation</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-slate-600">NPS</span>
                  <span className="text-sm font-medium text-cyan-700">${(aaPortfolioData.npsBalance / 100000).toFixed(1)}L</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-slate-600">Mutual Funds</span>
                  <span className="text-sm font-medium text-slate-700">${(aaPortfolioData.mutualFundsValue / 100000).toFixed(1)}L</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-slate-600">Equities</span>
                  <span className="text-sm font-medium text-slate-700">${(aaPortfolioData.equityValue / 100000).toFixed(1)}L</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-slate-600">Fixed Deposits</span>
                  <span className="text-sm font-medium text-slate-700">${(aaPortfolioData.fixedDepositsValue / 100000).toFixed(1)}L</span>
                </div>
              </div>
            </div>
          </Card>
        ...
        {showResults && (
          <Card className="p-6 mb-6 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <h3 className="text-sm font-medium text-gray-500 mb-1">Monthly investment Required</h3>
                <p className="text-2xl font-bold text-indigo-600">{formatCurrency(requiredMonthlyinvestment)}</p>
        ...
        <div className="space-y-6">
          <Card className="p-6">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Input
                    ...
                    className={`w-28 h-7 text-sm ${aaDataConnected ? 'border-emerald-200 bg-emerald-50' : ''}`}
                  />
                </div>
              </div>
              {aaDataConnected && (
                <p className="text-xs text-emerald-600 mb-2">
                  ...
                  Using real data from your banking transaction analysis
                </p>
              )}
              <Slider
            ...
            <div className="mb-4">
              ...
              {aaDataConnected && (
                <p className="text-xs text-emerald-600 mb-2">
                  <Database className="w-3 h-3 inline mr-1" />
                  Auto-calculated from your NPS, index funds, and long-term equity investments
                </p>
              )}
              <Slider
          ...
          <Card className="p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Choose the kind of life you want to live when you retire. This will change the amount of money you need to spend.
            </p>
          ...
          <Card className="p-6">
            ...
            <div className="mb-4">
              ...
              {aaDataConnected && (
                <p className="text-xs text-emerald-600 mb-2">
                  <Database className="w-3 h-3 inline mr-1" />
                  Based on your active investment investments from mutual fund analysis
                </p>
              )}
              <Slider
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1">
                  ...
                  <Tooltip text="The percentage by which you'll increase your monthly contribution each year (Step-up investment).">
                    <Info className="h-3.5 w-3.5 text-gray-400" />
                  </Tooltip>
                </div>
                <div className="flex items-center gap-1">
            ...
            <Card className="p-6 bg-gradient-to-r from-slate-50 to-gray-50 border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div className="bg-white p-4 rounded-lg border border-slate-200">
                  <h4 className="text-sm font-semibold text-slate-700 mb-2">Portfolio Allocation</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      <span>NPS (Retirement Focused)</span>
                      <span className="font-medium text-cyan-600">
                        {((aaPortfolioData.npsBalance / (aaPortfolioData.npsBalance + aaPortfolioData.mutualFundsValue + aaPortfolioData.equityValue + aaPortfolioData.fixedDepositsValue)) * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span>Mutual Funds</span>
                      <span className="font-medium text-slate-600">
                        {((aaPortfolioData.mutualFundsValue / (aaPortfolioData.npsBalance + aaPortfolioData.mutualFundsValue + aaPortfolioData.equityValue + aaPortfolioData.fixedDepositsValue)) * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span>Equities</span>
                      <span className="font-medium text-slate-600">
                        {((aaPortfolioData.equityValue / (aaPortfolioData.npsBalance + aaPortfolioData.mutualFundsValue + aaPortfolioData.equityValue + aaPortfolioData.fixedDepositsValue)) * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span>Fixed Deposits</span>
                      <span className="font-medium text-slate-600">
                        {((aaPortfolioData.fixedDepositsValue / (aaPortfolioData.npsBalance + aaPortfolioData.mutualFundsValue + aaPortfolioData.equityValue + aaPortfolioData.fixedDepositsValue)) * 100).toFixed(1)}%
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* Cash Flow Analysis */}
                <div className="bg-white p-4 rounded-lg border border-slate-200">
                  <h4 className="text-sm font-semibold text-slate-700 mb-2">Monthly Cash Flow</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      <span>Monthly Income</span>
                      <span className="font-medium text-emerald-600">${aaPortfolioData.monthlyIncome.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span>Monthly Expenses</span>
                      <span className="font-medium text-red-600">${aaPortfolioData.monthlyExpenses.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span>Current Investments</span>
                      <span className="font-medium text-slate-600">${aaPortfolioData.existinginvestments.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between text-xs border-t pt-1">
                      <span>Available Surplus</span>
                      <span className="font-medium text-emerald-600">
                        ${(aaPortfolioData.monthlyIncome - aaPortfolioData.monthlyExpenses - aaPortfolioData.existinginvestments).toLocaleString('en-IN')}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white p-4 rounded-lg border border-slate-200">
                <h4 className="text-sm font-semibold text-slate-700 mb-3">Personalized Suggestions</h4>
                <div className="space-y-3">
                  {aaPortfolioData.npsBalance < 500000 && (
                    <div className="flex items-start space-x-2">
                      <AlertTriangle className="h-4 w-4 text-amber-500 mt-0.5 flex-shrink-0" />
                      <p className="text-xs text-slate-600">
                        You have ${aaPortfolioData.npsBalance.toLocaleString('en-IN')} in your NPS account. Think about raising your NPS contributions to ${Math.min(102000, aaPortfolioData.monthlyIncome * 0.1 * 12).toLocaleString('en-IN')} a year so you can get tax breaks under 80CCD.
                      </p>
                    </div>
                  )}
                  
                  {(aaPortfolioData.monthlyIncome - aaPortfolioData.monthlyExpenses - aaPortfolioData.existinginvestments) > 10000 && (
                    <div className="flex items-start space-x-2">
                      <Target className="h-4 w-4 text-emerald-500 mt-0.5 flex-shrink-0" />
                      <p className="text-xs text-slate-600">
                        You have ${(aaPortfolioData.monthlyIncome - aaPortfolioData.monthlyExpenses - aaPortfolioData.existinginvestments).toLocaleString('en-IN')} left over each month. Think about putting an extra ${Math.floor((aaPortfolioData.monthlyIncome - aaPortfolioData.monthlyExpenses - aaPortfolioData.existinginvestments) * 0.7 / 1000) * 1000} into your retirement accounts to speed up the growth of your corpus.
                      </p>
                    </div>
                  )}

                  {((aaPortfolioData.fixedDepositsValue / (aaPortfolioData.npsBalance + aaPortfolioData.mutualFundsValue + aaPortfolioData.equityValue + aaPortfolioData.fixedDepositsValue)) * 100) > 40 && (
                    <div className="flex items-start space-x-2">
                      <TrendingUp className="h-4 w-4 text-slate-500 mt-0.5 flex-shrink-0" />
                      <p className="text-xs text-slate-600">
                        There are {((aaPortfolioData.fixedDepositsValue / (aaPortfolioData.npsBalance + aaPortfolioData.mutualFundsValue + aaPortfolioData.equityValue + aaPortfolioData.fixedDepositsValue)) * 100).toFixed(1)}% of your investments in fixed deposits. If you want to grow your money for retirement over the long term, think about slowly moving some of your FD money into equity index funds that adjust for inflation.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <p className="text-xs text-slate-600">
                      Your current retirement corpus of ${aaPortfolioData.currentRetirementCorpus.toLocaleString('en-IN')} is {((aaPortfolioData.currentRetirementCorpus / (targetCorpus || 1)) * 100).toFixed(1)}% of your target corpus, which is {retirementAge - currentAge} years away.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {showResults && (
        <div className="mt-10 space-y-8">
                ...
                <Button 
                  ...
                  onClick={() => {
                    ...
                    toast({
                      ...
                      description: "Increased monthly investment by 25% to see the impact.",
                    });
                  }}
                >
                  ...
                  <p className="text-sm font-medium">Increase investment by 25%</p>
          ...
          <section className="mb-8">
            <div className="mb-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Smart Retirement Planning Tips</h3>
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-semibold text-gray-800 mb-2">📊 Look at Different Situations</h4>
                  <p className="text-sm text-gray-600">Change the input values to see how they change the results. This can help you make better choices about money and understand the impact of different scenarios on your retirement planning.</p>
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-gray-800 mb-2">🔄 Updates on a Regular Basis</h4>
                  <p className="text-sm text-gray-600">The rules about money change over time. When interest rates, inflation, or other things change, don't forget to recalculate your retirement needs to stay on track.</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <div>
                <h4 className="text-sm font-semibold text-gray-800 mb-2">💰 Optimize Your Investment</h4>
                <p className="text-sm text-gray-600">
                  {shortfallOrSurplus < 0 ? 
                    `Increase your monthly investment by $${Math.abs(Math.ceil((requiredMonthlyinvestment - monthlyContribution) / 500) * 500).toLocaleString('en-IN')} to reach your target and close the retirement gap.` :
                    `You're on track! Consider investing $${Math.ceil(monthlyContribution * 0.2 / 500) * 500} more monthly for additional security and early retirement options.`
                  }
                </p>
              </div>
              
              <div>
                <h4 className="text-sm font-semibold text-gray-800 mb-2">🎯 Adjust Retirement Age</h4>
                <p className="text-sm text-gray-600">
                  {shortfallOrSurplus < 0 ? 
                    `Working ${Math.min(5, Math.ceil(Math.abs(shortfallOrSurplus) / targetCorpus * 10))} more years can significantly improve your retirement readiness and financial security.` :
                    `You could retire up to ${Math.min(3, Math.floor(shortfallOrSurplus / targetCorpus * 10))} years earlier than planned given your current corpus and savings rate.`
                  }
                </p>
              </div>
            </div>
            ...
            <div className="mb-6">
              <h4 className="text-sm font-semibold text-gray-800 mb-3">⚖️ Recommended Allocation Strategy</h4>
              <div className="mb-4">
              ...
              <p className="text-sm text-gray-600">
                {retirementAge - currentAge > 15 ?
                  "With over 15 years to retirement, consider an aggressive equity-focused portfolio (65-70% equity, 20-25% debt, 5-10% gold) for maximum growth potential." :
                  retirementAge - currentAge > 7 ?
                    "With 7-15 years to retirement, a balanced portfolio (50% equity, 40% debt, 10% gold) offers growth with reduced volatility." :
                    "With less than 7 years to retirement, a conservative portfolio (30% equity, 60% debt, 10% gold) helps preserve capital."
                }
              </p>
            </div>
            
            <div className="mb-6">
              <h4 className="text-sm font-semibold text-gray-800 mb-3">⏰ Time-Sensitive Actions</h4>
              
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-medium text-gray-800">✓ Max out EPF/PPF</p>
                    <p className="text-sm text-gray-600">Take advantage of tax benefits and guaranteed returns</p>
                  </div>
                  
                  <div>
                    <p className="text-sm font-medium text-gray-800">✓ Set up automatic investments</p>
                    <p className="text-sm text-gray-600">Build discipline and consistency in wealth building</p>
                  </div>
                </div>
                
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-medium text-gray-800">✓ Review your plan yearly</p>
                    <p className="text-sm text-gray-600">Stay on track with your retirement goals and adjustments</p>
                  </div>
                  
                  <div>
                    <p className="text-sm font-medium text-gray-800">✓ Get adequate insurance</p>
                    <p className="text-sm text-gray-600">Protect your retirement savings from unexpected events</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section className="mb-8">
            <h3 className="text-xl font-bold text-gray-900 mb-6">Benefits and Challenges of Retirement Planning</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <section className="bg-green-50 p-4 rounded-lg">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-semibold text-green-700 text-base">Benefits of Early Planning</h4>
                </div>
                <ul className="space-y-2 text-sm text-gray-700">
                  <li className="flex gap-2"><span className="text-green-600 font-medium">✓</span> <span><strong>Power of Compounding:</strong> Starting early allows your investments to grow exponentially over time.</span></li>
                  <li className="flex gap-2"><span className="text-green-600 font-medium">✓</span> <span><strong>Lower Monthly Burden:</strong> Spread the savings requirement over more years reduces monthly contribution needs.</span></li>
                  <li className="flex gap-2"><span className="text-green-600 font-medium">✓</span> <span><strong>Financial Flexibility:</strong> Multiple income streams and investment options become available.</span></li>
                  <li className="flex gap-2"><span className="text-green-600 font-medium">✓</span> <span><strong>Peace of Mind:</strong> Systematic planning reduces financial stress and uncertainty.</span></li>
                  <li className="flex gap-2"><span className="text-green-600 font-medium">✓</span> <span><strong>Independence:</strong> Self-reliance without depending on family or government support.</span></li>
                </ul>
              </section>
              ...
              <section className="bg-red-50 p-4 rounded-lg">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-semibold text-red-700 text-base">Planning Challenges</h4>
                </div>
                <ul className="space-y-2 text-sm text-gray-700">
                  <li className="flex gap-2"><span className="text-red-600 font-medium">✗</span> <span><strong>Inflation Risk:</strong> Rising costs can erode purchasing power over decades.</span></li>
                  <li className="flex gap-2"><span className="text-red-600 font-medium">✗</span> <span><strong>Market Volatility:</strong> Investment returns can vary significantly with market conditions.</span></li>
                  <li className="flex gap-2"><span className="text-red-600 font-medium">✗</span> <span><strong>Discipline Required:</strong> Consistent savings over 20-30 years requires strong financial discipline.</span></li>
                  <li className="flex gap-2"><span className="text-red-600 font-medium">✗</span> <span><strong>Opportunity Cost:</strong> Money saved for retirement can't be used for current lifestyle needs.</span></li>
                  <li className="flex gap-2"><span className="text-red-600 font-medium">✗</span> <span><strong>Healthcare Uncertainties:</strong> Medical costs in old age can be unpredictable and substantial.</span></li>
                </ul>
              </section>
            </div>
          </section>
          ...
          <section className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-12 px-6 text-center mt-12">
            <div className="max-w-4xl mx-auto">
              <h3 className="text-2xl font-bold mb-4">Start Planning Your Retirement Today</h3>
              <p className="text-blue-100 text-base mb-6 max-w-2xl mx-auto">
                Every year you delay retirement planning increases the monthly savings required. Use our comprehensive calculator to map your retirement journey and achieve financial independence.
              </p>
              <Button 
