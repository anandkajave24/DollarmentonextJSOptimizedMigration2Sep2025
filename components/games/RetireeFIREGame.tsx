...
import { TrendingUp, TrendingDown, DollarSign, Brain, Heart, AlertTriangle, Home, Activity } from 'lucide-react';
import { retireeStages, type RetireeGameStage, type RetireeStageOption } from '@/data/retireeStages';
...
const RetireeFIREGame: React.FC = () => {
  ...
  const currentStageData = retireeStages[gameState.currentStage];

  const handleChoice = (option: RetireeStageOption) => {
    const newState = {
      ...
      healthcareCosts: Math.max(0, gameState.healthcareCosts + option.impact.healthcareCosts),
      withdrawal: Math.max(0, gameState.withdrawal + option.impact.withdrawal),
      gameCompleted: gameState.currentStage + 1 >= retireeStages.length,
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-4 bg-white rounded-lg shadow">
                ...
                <div className="text-2xl font-bold text-red-600">{formatCurrency(gameState.healthcareCosts)}</div>
                <div className="text-sm text-gray-600">Healthcare Costs</div>
              </div>
              <div className="text-center p-4 bg-white rounded-lg shadow">
              ...
              <div className="text-center p-4 bg-white rounded-lg shadow">
                ...
                <div className="text-2xl font-bold text-pink-600">{gameState.emotion}/10</div>
                <div className="text-sm text-gray-600">Life Satisfaction</div>
              </div>
            </div>

            <div className="text-center space-y-4">
              <div className="text-lg text-gray-700">
                Congratulations! You've successfully navigated the complexities of retirement, managing RMDs, healthcare costs, and estate planning.
              </div>
              <div className="text-md text-gray-600">
                Your thoughtful decisions about withdrawal strategies, Medicare supplements, and legacy planning have ensured a secure and meaningful retirement.
              </div>
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <Card className="border-blue-200 bg-blue-50">
        <CardHeader>
          <CardTitle className="text-xl text-blue-800 flex items-center gap-2">
            ...
            Retirement Journey - Stage {gameState.currentStage + 1} of {retireeStages.length}
          </CardTitle>
          <div className="w-full bg-blue-200 rounded-full h-2">
            <div 
              className="bg-blue-600 h-2 rounded-full transition-all duration-500" 
              style={{ width: `${((gameState.currentStage + 1) / retireeStages.length) * 100}%` }}
            />
          </div>
        </CardHeader>
      </Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <Activity className="w-8 h-8 mx-auto mb-2 text-red-600" />
            <div className="text-lg font-bold">{formatCurrency(gameState.healthcareCosts)}</div>
            <div className="text-sm text-gray-600">Healthcare Costs</div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <Heart className={`w-8 h-8 mx-auto mb-2 ${getScoreColor(gameState.emotion)}`} />
            <div className={`text-lg font-bold ${getScoreColor(gameState.emotion)}`}>{gameState.emotion}/10</div>
            <div className="text-sm text-gray-600">Life Satisfaction</div>
          </CardContent>
        </Card>
      </div>
            ...
            <CardContent className="p-6 text-center">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Net Worth:</span>
                  <span className={gameState.netWorth > 1875000 ? 'text-green-600' : 'text-red-600'}>
                    {formatCurrency(gameState.netWorth)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Healthcare Costs:</span>
                  <span className="text-red-600">{formatCurrency(gameState.healthcareCosts)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Knowledge:</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className={getScoreColor(gameState.emotion)}>{gameState.emotion}/10</span>
                </div>
              </div>
...
export default RetireeFIREGame;