...
const FreelancerFIREGame: React.FC<FreelancerFIREGameProps> = ({ 
  ...
  const getDynamicContext = (stageId: number) => {
    if (stageId === 2) { // Client Payment Crisis
      ...
      if (firstDecision?.choice.includes("Survival Mode")) {
        return "⚠️ SURVIVAL MODE CONSEQUENCES: Your decision to take any project at low rates has attracted clients who don't value your work. They delay payments because they see you as desperate and replaceable.";
  ...
  const generateFreelancerFIREAssessment = () => {
    ...
    if (netWorth >= 5000000) {
      ...
      personalizedGuidance = `You're ₹${((5000000 - netWorth) / 100000).toFixed(1)}L away from FIRE. Your freelance experience gives you flexibility to reach the goal.`;
  ...
  const calculateFreelancerFIREProgress = () => {
    ...
    const debtImpact = totalDebt / 100000;
...
${netWorth >= 5000000 ? 
  ...
  `You need ₹${((5000000 - netWorth)/100000).toFixed(1)}L more for FIRE. Consider productizing your services or building passive income streams.`
}`;
  ...
  if (gameState.currentStage >= freelancerStages.length) {
    ...
    if (gameState.decisions.find(d => d.choice.includes("Doormat"))) {
      missedOpportunities.push("Client Management: Poor payment enforcement cost you ₹10-15L in delayed/lost payments");
    }
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <div className="text-sm text-gray-600">Age {gameState.age} • ₹{(gameState.monthlyIncome/1000).toFixed(0)}K/month</div>
            ...
            >
              ...
              {!showFeedback ? (
                ...
                <div className="space-y-6">
                  {selectedOption && (() => {
                    const option = currentStageData.options.find(opt => opt.id === selectedOption)!;
                    const isGoodChoice = option.impact.netWorth >= 0 && option.impact.independence >= 0;
