...
const HousewifeFIREGame: React.FC<HousewifeFIREGameProps> = ({ 
  ...
  const getDynamicContext = (stageId: number) => {
    if (stageId === 2) { // First Pregnancy Financial Planning
      ...
      if (firstDecision?.choice.includes("Traditional")) {
        return "⚠️ TRADITIONAL CHOICE IMPACT: Your decision to remain financially dependent is now creating stress as pregnancy expenses mount and you have no personal financial resources to contribute.";
  ...
  const generateHousewifeFIREAssessment = () => {
    ...
    if (netWorth >= 3000000 && gameState.independenceScore >= 15) {
    ...
    } else if (netWorth >= 2000000) {
      fireStatus = "⚡ FAMILY-SUPPORTED FINANCIAL SECURITY";
      ...
      fireStatus = "⚠️ FINANCIAL VULNERABILITY";
  ...
  const calculateHousewifeFIREProgress = () => {
    ...
    const fireTarget = 3000000; // Lower target considering family context
...
📊 **YOUR 34-YEAR FAMILY & FINANCIAL JOURNEY ASSESSMENT**

${generateHousewifeFIREAssessment()}

**CURRENT FINANCIAL POSITION:**
• Personal Net Worth: ₹${(netWorth/100000).toFixed(1)} Lakhs
...
${independenceScore >= 15 && familyHappinessScore >= 15 ? 
  ...
  if (gameState.currentStage >= housewifeStages.length) {
    ...
    if (gameState.decisions.find(d => d.choice.includes("Traditional") && d.stage.includes("Invisibility"))) {
      missedOpportunities.push("Financial Independence: Traditional approach limited earning potential by ₹30-50L over 34 years");
    }
