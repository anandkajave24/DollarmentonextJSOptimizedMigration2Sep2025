...
const SalariedProfessionalFIREGame: React.FC<SalariedProfessionalFIREGameProps> = ({ 
  ...
  const getDynamicContext = (stageId: number) => {
    if (stageId === 5) { // Relationship & Money stage
      ...
      if (skillInvestmentDecision?.choice.includes("Decline Course")) {
        return "⚠️ CAREER STAGNATION IMPACT: Your decision to skip the ₹50K certification has limited your salary growth to ₹75K/month instead of potential ₹1.2L. This conservative approach is now affecting your financial capacity for major life decisions. Your partner notices the financial constraints.";
      } else if (skillInvestmentDecision?.choice.includes("Investment")) {
        return "🚀 CAREER GROWTH ACHIEVED: Your ₹50K investment in certification paid off! Your salary jumped to ₹1.1L/month, giving you financial confidence for major life decisions. Your partner is impressed by your strategic thinking.";
      }
  ...
  const generateFIREAssessment = () => {
    ...
    if (netWorth >= 5000000) {
      ...
      personalizedGuidance = "Your disciplined approach throughout 26 years has paid off. You've successfully built ₹5+ crores and achieved financial independence.";
    } else if (netWorth >= 3000000) {
      fireStatus = "⚡ CLOSE TO FIRE - Need 2-3 More Years";
      personalizedGuidance = `You're ₹${((5000000 - netWorth) / 100000).toFixed(1)}L away from FIRE. Your journey has been mostly positive, but some decisions slowed your progress.`;
  ...
  const calculateRealFIREProgress = () => {
    ...
    const fireTarget = 5000000; // ₹5 Crores
    ...
    const debtImpact = totalDebt / 100000; // Debt reduces progress
  ...
  const getFinalStageContent = () => {
    ...
    if (careerMistake) {
      specificMistakes.push("❌ Stage 4: Declined ₹50K skill investment - Cost you ₹15-20L in lifetime earnings");
    ...
    if (weddingMistake) {
      specificMistakes.push("❌ Stage 5: ₹8L wedding debt - This ₹8L could have become ₹80L by age 50");
      actionableAdvice.push("✅ Celebrate meaningfully but don't mortgage your future for one day");
    }
    
    const dynamicSituation = `
📊 **YOUR 26-YEAR FINANCIAL JOURNEY ASSESSMENT**

${assessment.fireStatus}

**CURRENT FINANCIAL POSITION:**
• Net Worth: ₹${(netWorth/100000).toFixed(1)} Lakhs
• Total Debt: ₹${(totalDebt/100000).toFixed(1)} Lakhs  
• FIRE Target: ₹50 Lakhs (Financial Independence)
• Gap: ₹${((5000000 - netWorth)/100000).toFixed(1)} Lakhs remaining
...
Your early decisions had massive compound effects. That ₹8K emergency fund from Stage 1 could have grown to ₹2L+ by now. Every emotional decision cost you years of FIRE progress.

${assessment.personalizedGuidance}

**REALITY CHECK:**
${netWorth >= 5000000 ? 
  "You can now generate ₹15-20L annually from your corpus. Work is now optional!" :
  `You need ₹${((5000000 - netWorth)/100000).toFixed(1)}L more for FIRE. At current trajectory, you'll achieve FIRE by age ${Math.ceil(50 + (5000000 - netWorth) / (gameState.salary * 12 * 0.2))}.`
}`;
  ...
  const proceedToNextStage = () => {
    ...
    setGameState(prev => {
      ...
      if (newState.netWorth >= 1000000 && !newAchievements.includes('First Crore')) {
        newAchievements.push('First Crore');
      }
  ...
  const calculateFIREProgress = () => {
    const fireTarget = 5000000; // ₹5 Crores for FIRE
  ...
  if (gameState.currentStage >= salariedProfessionalStages.length) {
    ...
    if (gameState.decisions.find(d => d.choice.includes("Decline Course"))) {
      missedOpportunities.push("Career Growth: Skipping ₹50K certification cost you ₹15-20L in lifetime earnings");
    }
    if (gameState.decisions.find(d => d.choice.includes("Party") || d.choice.includes("Live Like"))) {
      missedOpportunities.push("First Salary Discipline: Lifestyle inflation from day 1 cost you ₹5-8L over 26 years");
    }
    if (gameState.decisions.find(d => d.choice.includes("Grand Wedding"))) {
      missedOpportunities.push("Wedding Expenses: ₹8L wedding debt cost you ₹25L+ in compound interest");
    }
    if (gameState.decisions.find(d => d.choice.includes("FOMO") || d.choice.includes("Panic"))) {
      missedOpportunities.push("Investment Timing: Emotional decisions cost you 3-5% annual returns = ₹10-15L");
    }
    ...
    if (gameState.netWorth < 2000000) {
      ...
      improvementAreas.push("Investment Start: Starting SIP 6 months earlier = ₹3-5L difference");
    }
    if (gameState.totalDebt > 500000) {
      improvementAreas.push("Debt Management: High-interest debt cost you ₹5-8L in unnecessary interest");
    }
    ...
    if (gameState.netWorth >= 5000000) {
      ...
      actionPlan.push("Close to FIRE! Increase SIP by ₹10K monthly to achieve FIRE in 2-3 years");
      ...
      actionPlan.push("Start aggressive systematic investing - ₹15K+ monthly SIP");
            ...
            <div className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-red-50 to-red-100 border-2 border-red-300 rounded-xl p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-red-800">Critical Mistakes That Cost You Crores</h2>
                ...
                {missedOpportunities.length > 0 ? (
                  ...
                  <div className="bg-green-100 border border-green-300 p-6 rounded-lg text-center">
                    ...
                    <h3 className="text-xl font-bold text-green-800 mb-2">Excellent Decision Making!</h3>
                    <p className="text-green-700">You avoided all major financial mistakes throughout your 26-year journey.</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gradient-to-r from-yellow-50 to-orange-100 border-2 border-yellow-300 rounded-xl p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-yellow-600">Small improvements that could have added lakhs to your wealth</p>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="bg-white p-5 rounded-lg shadow-md border-l-4 border-yellow-500">
                    ...
                    <p className="text-sm text-gray-700 mb-2">Earlier skill investment could have reached ₹2L+ monthly salary by age 35</p>
                    <div className="text-xs bg-yellow-50 text-yellow-700 px-2 py-1 rounded">
                      Potential Impact: ₹15-20L additional lifetime earnings
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-lg shadow-md border-l-4 border-blue-500">
                    ...
                    <p className="text-sm text-gray-700 mb-2">Starting SIP from month 1 instead of month 6</p>
                    <div className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded">
                      Potential Impact: ₹3-5L additional corpus
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-lg shadow-md border-l-4 border-green-500">
                    ...
                    <div className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded">
                      Potential Impact: ₹5-8L saved in interest costs
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-50 to-purple-100 border-2 border-purple-300 rounded-xl p-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    ...
                    <p className="text-xs text-gray-600">₹50K skill investment = ₹20L lifetime returns</p>
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    <div className="text-green-600 font-bold mb-2">✅ Stage 7: Stay Invested During Crashes</div>
                    <p className="text-xs text-gray-600">Increase SIP when markets fall by 30%</p>
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    <div className="text-green-600 font-bold mb-2">✅ Stage 8: Term + Mutual Fund SIP</div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              ...
              <div className="text-lg font-bold text-green-600">₹{(gameState.netWorth/100000).toFixed(1)}L Net Worth</div>
            </div>
          </div>
